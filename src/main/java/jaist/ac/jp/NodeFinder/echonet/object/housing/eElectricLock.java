...
import jaist.ac.jp.NodeFinder.util.EchonetDataConverter.AlarmStatus;
...
public class eElectricLock extends eDataObject{
	...
	public eElectricLock(Node node,EOJ eoj) {
		...
		setType(DeviceIDEnum.ElectricLock);
	}
			...
			service.doGet(this.getNode(), this.getEoj(), epcs, 5000, new GetListener() {
			    ...
			    public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					switch (resultData.getEPC()) 
					{
						...
						break;
						case xE5:
							refreshAlarmStatus(EchonetDataConverter.AlarmStatus.fromCode(resultData.toBytes()[0]));
