...
public class eGasLeakSensor extends eDataObject{
	...
	public eGasLeakSensor(Node node,EOJ eoj) {
		...
		setType(eDeviceType.GasLeakSensor);
	}
			    ...
			    public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					switch (resultData.getEPC()) 
					{
						case xB1:
							...
							logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={GasLeakedDetect:%s}",
						...
						case xB0:
							...
							logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={GasThresholdLevel:%s}",
			...
			service.doObserve(getNode(), getEoj(), epcs, new ObserveListener() {
			    ...
			    public void receive(ObserveResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xB1:
						...
						logger.info(String.format("OBSERVER: Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={GasLeakedDetect:%s}",
								 getNode().getNodeInfo().toString(),getEoj().toString(),resultData.toBytes()[0],isGasLeakOccurenceStatus()));
						break;
					default:
						break;
