...
public class EPC0x86Handler {
	private static final Logger logger = Logger.getLogger(EPC0x86Handler.class.getName());
	public static void refreshManufacturerFaultCode(Object obj, String newVal) {
		...
		if(obj instanceof eSuperObject) {
			...
			if(!newVal.equals(dev.getManufacturerFaultCode())) {
				dev.setManufacturerFaultCode(newVal);
				eTrigger.manufacturerFaultCodeChanged(dev);
			}
		}else {
