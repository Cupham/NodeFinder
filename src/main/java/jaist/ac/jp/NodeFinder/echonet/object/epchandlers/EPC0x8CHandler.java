...
public class EPC0x8CHandler {
	private static final Logger logger = Logger.getLogger(EPC0x8CHandler.class.getName());
	public static void refreshProductCode(Object obj, String newVal) {
		if(obj instanceof eSuperObject) {
			...
			if(!newVal.equals(dev.getProductCode())) {
				dev.setProductCode(newVal);
				eTrigger.productCodeChanged(dev);
			}
		}else {
