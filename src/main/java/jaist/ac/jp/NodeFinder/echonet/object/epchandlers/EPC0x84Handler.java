...
public class EPC0x84Handler {
	private static final Logger logger = Logger.getLogger(EPC0x84Handler.class.getName());
	public static void refreshInstantaneousPower(Object obj, short newVal) {
		...
		if(obj instanceof eSuperObject) {
			if(obj instanceof eSuperObject) {
				...
				if(dev.getInstantaneousPower() != newVal) {
					dev.setInstantaneousPower(newVal);
					eTrigger.instantaneousPowerChanged(dev);
				}
			}else {
