...
public class EPC0x85Handler {
	private static final Logger logger = Logger.getLogger(EPC0x85Handler.class.getName());
	public static void refreshCumulativePower(Object obj, long newVal) {
		if(obj instanceof eSuperObject) {
			if(obj instanceof eSuperObject) {
				...
				if(dev.getCumulativePower() != newVal) {
					dev.setCumulativePower(newVal);
					eTrigger.cumulativePowerChanged(dev);
				}
			}else {
