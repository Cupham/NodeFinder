...
public class eElectricHeater extends eDataObject{
	private static final Logger logger = Logger.getLogger(eElectricHeater.class.getName());
	private Timer timer;
	private int settingTemperature;
	
	

	public eElectricHeater(Node node,EOJ eoj) {
		super(node, eoj);
		this.setGroupCode((byte) 0x01);
		this.setClassCode((byte) 0x42);
		this.setInstanceCode(eoj.getInstanceCode());
		setType(DeviceIDEnum.ElectricHeater);
	}
