...
public class ePassageSensor extends eDataObject{
	...
	public ePassageSensor(Node node,EOJ eoj) {
		...
		setType(eDeviceType.PassageSensor);
	}
		...
		try {
			service.doGet(this.getNode(), this.getEoj(), epcs, 5000, new GetListener() {
			    ...
			    public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					{
						case xE0:
							refreshPassageDetectionDirection(eConverter.dataToDirection(resultData));
						...
						break;
						case xBE:
							refreshPassageDetectionHoldingTime(eConverter.dataToInteger(resultData) * 10);
