...
import jaist.ac.jp.NodeFinder.util.EchonetDataConverter.SprinklerValveOpenCloseSetting;
...
public class eGardenSprinkler extends eDataObject{
	...
	public eGardenSprinkler(Node node,EOJ eoj) {
		...
		setType(DeviceIDEnum.GardenSprinkler);
	}
		...
		try {
			service.doGet(this.getNode(), this.getEoj(), epcs, 5000, new GetListener() {
			    ...
			    public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					{
						case xE0:
							refreshSprinklerValveOpenCloseSetting(EchonetDataConverter.SprinklerValveOpenCloseSetting.fromCode(resultData.toBytes()[0]));
