...
public class EPC0x9AHandler {
	private static final Logger logger = Logger.getLogger(EPC0x9AHandler.class.getName());
	public static void refreshCumulativeTime(Object obj, String newVal) {
		if(obj instanceof eSuperObject) {
			...
			if(!newVal.equals(dev.getCumulativeTime())) {
				dev.setCumulativeTime(newVal);
				eTrigger.cumulativeTimeChanged(dev);
			}
		}else {
