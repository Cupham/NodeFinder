...
public class EPC0x87Handler {
	private static final Logger logger = Logger.getLogger(EPC0x87Handler.class.getName());
	public static void refreshCurrentLimitSetting(Object obj, int newVal) {
		...
		if(obj instanceof eSuperObject) {
			...
			if(dev.getCurrentLimitSetting() != newVal) {
				dev.setCurrentLimitSetting(newVal);
				//TODO: More operation can be added here
				eTrigger.currentLimitSettingChanged(dev);
			}
		}else {
