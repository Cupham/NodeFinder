...
public class eEarthquakeSensor extends eDataObject{
	...
	public eEarthquakeSensor(Node node,EOJ eoj) {
		...
		setType(DeviceIDEnum.EarthquakeSensor);
	}
			    ...
			    public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					switch (resultData.getEPC()) 
					{
						case xB1:
							...
							logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={EarthquakeDetect:%s}",
						...
						case xB0:
							...
							logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={EarthquakeDetectionThresholdLevel:%s}",
									 getNode().getNodeInfo().toString(),getEoj().toString(),resultData.toBytes()[0],getDetectionThresholdLevel()));
						break;
						case xC0:
							refreshSiValue(EchonetDataConverter.dataToInteger(resultData)*0.1f);
			...
			service.doObserve(getNode(), getEoj(), epcs, new ObserveListener() {
			    ...
			    public void receive(ObserveResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xB1:
						...
						logger.info(String.format("OBSERVER: Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={EarthquakeDetect:%s}",
								 getNode().getNodeInfo().toString(),getEoj().toString(),resultData.toBytes()[0],isEarthquakeOccurrenceStatus()));
					break;
					case xC0:
						refreshSiValue(EchonetDataConverter.dataToInteger(resultData)*0.1f);
