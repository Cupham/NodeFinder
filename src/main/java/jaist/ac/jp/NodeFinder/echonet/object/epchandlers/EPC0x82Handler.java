...
public class EPC0x82Handler {
	private static final Logger logger = Logger.getLogger(EPC0x82Handler.class.getName());
	public static void refreshStandardVersionInfo(Object obj, String newVal) {
		...
		if(obj instanceof eSuperObject) {
			...
			if(!newVal.equals(dev.getStandardVersionInfo())) {
				dev.setStandardVersionInfo(newVal);
				eTrigger.standardVersionInforChanged(dev);
			}
		}else {
